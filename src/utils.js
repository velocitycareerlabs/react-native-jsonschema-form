...
export const getRequiredAndNotHiddenFields = (requiredFileds, uiSchema) => (
  Object.keys(requiredFileds || {}).reduce((acc, key) => {
    const uiSchemaForKey = key.split('.').reduce((subAcc, subKey) => {
      if ((subAcc.uiSchema[subKey] || {})['ui:widget'] === 'hidden') {
        return { ...subAcc, ...{ uiSchema: subAcc.uiSchema[subKey] || {}, subKeyRequired: false } };
      }
      return { ...subAcc, ...{ uiSchema: subAcc.uiSchema[subKey] || {} } };
